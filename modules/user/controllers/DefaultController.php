    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index', 'confirm', 'resend', 'logout', 'login', 'register'],
                        'allow' => true,
                        'roles' => ['?', '@'],
                    ],
                    [
                        'actions' => ['account', 'profile', 'resend-change', 'cancel', 'upload_avatar'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                    [
                        'actions' => ['forgot', 'reset', 'login-email', 'login-callback'],
                        'allow' => true,
                        'roles' => ['?'],
                    ],
                ],
...
//        echo Yii::$app->security->generatePasswordHash('123456'); die();
    ...
    public function actionRegister()
    {
        ...
        if ($user->load($post)) {
            ...
            if ($user->validate() && $profile->validate()) {
                ...
                $profile->full_name = $user->getDisplayName();
                $profile->setUser($user->id)->save();
                $this->afterRegister($user);

                // set flash
                // don't use $this->refresh() because user may automatically be logged in and get 403 forbidden
                $successText = "Đăng ký thành công [ " . $user->getDisplayName() . " ]";
                $guestText = "";
                if (Yii::$app->user->isGuest) {
                    $guestText = Yii::t("user", " - Please check your email to confirm your account");
                }
                Yii::$app->session->setFlash("Register-success", $successText . $guestText);
                return $this->redirect('/thong-tin-ca-nhan');
            }
        }
    ...
    public function actionUpload_avatar(){
        ...
        if (Yii::$app->request->isPost) {
            ...
            $imageSize = getimagesize($model->picture->tempName);
            if($imageSize[0] < 200){
                $returnData = ['error' => 1, 'message' => "Kích cỡ ảnh tối thiểu 200x200"];
                return $returnData;
            }

            if ($model->picture) {
                $dirName = "uploads/user_avatar";


                $fileName = $dirName . "/tmp/" . Yii::$app->user->id . ".png";
                $avatarName = $dirName . "/" . Yii::$app->user->id . ".png";

                $model->picture->saveAs($fileName);


                  //  Image::thumbnail($fileName, 64, 64, ManipulatorInterface::THUMBNAIL_INSET)->save($thumb64, ['quality' => 100]);
                Image::thumbnail($fileName, 110, 110, ManipulatorInterface::THUMBNAIL_OUTBOUND)->save($avatarName, ['quality' => 100]);

                $returnData = ['error' => 0];
                return $returnData;
            }else{
                $returnData = ['error' => 1, 'message' => "Ảnh không đúng định dạng hoặc dung lượng quá lớn"];
                return $returnData;
            }
        }
        $returnData = ['error' => 1, 'message' => ""];
        return $returnData;
    }
}